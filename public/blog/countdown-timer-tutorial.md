...
categories:
  - tutorial
  - business
excerpt: Learn the PRP/SpecKit workflow by building a countdown timer. From requirements to production code with HogBall template.
...
ogDescription: Learn the PRP/SpecKit workflow by building a countdown timer. Step-by-step tutorial from requirements to production-ready code with HogBall.
...
HogBall isn't just a Next.js template—it's your **entry point** to client relationships.
...
- Theme customization (most brands just pick light/dark, though HogBall includes 32 themes)
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3">
          <div className="text-center">
            ...
            <div className="text-sm">Custom HogBall Setup</div>
          </div>
...
docker compose exec hogball pnpm run format

# Run full test suite and build
docker compose exec hogball sh -c "pnpm run test:suite && pnpm run build"
```
...
**Test**: `docker compose exec hogball sh -c "pnpm run test:suite && pnpm run build"`
...
This tutorial demonstrates **building a consulting business** using HogBall:
...
Clients want proven solutions. Developers want starting points. Consultants want leverage. HogBall gives you all three.
...
**Next Move**: [Fork HogBall](https://github.com/TortoiseWolfe/HogBall/fork) → Deploy countdown → Share on LinkedIn → Book first client
