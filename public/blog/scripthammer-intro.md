...
featuredImage: /blog-images/hogball-intro/featured-og.svg
featuredImageAlt: Scripthammer - The Opinionated Next.js PWA Template with 32 themes and Docker development
ogImage: /blog-images/hogball-intro/featured-og.png
...
![Scripthammer Dashboard](/blog-images/hogball-intro/dashboard-overview.svg)
...
![Testing Suite Output](/blog-images/hogball-intro/testing-output.svg)
...
![Docker Architecture](/blog-images/hogball-intro/docker-architecture.svg)
_Docker-first development environment with isolated containers_

**⚠️ CRITICAL**: HogBall is Docker-only. Local npm/pnpm commands are NOT supported and WILL NOT WORK.
...
```bash
docker compose exec hogball pnpm run generate:component MyComponent atomic
```
...
```bash
# Quick validation
docker compose exec hogball pnpm run test:quick
...
- ✅ `docker compose exec hogball pnpm install` - Correct
- ❌ `pnpm install` - Will NOT work
- ❌ `npm install` - Will NOT work

## 🎯 What's Next?

Check out the [CONSTITUTION.md](https://github.com/TortoiseWolfe/HogBall/blob/main/.specify/memory/constitution.md) for the project principles and current sprint goals. Read [CLAUDE.md](https://github.com/TortoiseWolfe/HogBall/blob/main/CLAUDE.md) for AI (Artificial Intelligence) pair programming guidelines and best practices when working with this codebase.
