// HogBall Service Worker v1.0.0
// Provides offline support, caching, and background sync
// Note: Cache name includes project name - updated by rebrand script

const CACHE_VERSION = 'hogball-v1.0.0'; // Updated by scripts/rebrand.sh
  ...
  if (event.data && event.data.type === 'CLEAR_CACHE') {
    event.waitUntil(
      caches.keys().then((cacheNames) => {
        return Promise.all(
          cacheNames
            .filter((cacheName) => cacheName.startsWith('hogball-'))
            .map((cacheName) => caches.delete(cacheName))
        );
      })
    );
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification('HogBall Notification', options)
  );
});
