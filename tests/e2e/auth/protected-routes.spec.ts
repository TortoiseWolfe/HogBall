...
test.describe('Protected Routes E2E', () => {
  const testEmail = `e2e-protected-${Date.now()}@tortoisewolfe.com`;
  ...
  test('should enforce RLS policies on payment access', async ({ page }) => {
    ...
    const user1Email = `e2e-rls-1-${Date.now()}@tortoisewolfe.com`;
    await page.goto('/sign-up');
    ...
    const user2Email = `e2e-rls-2-${Date.now()}@tortoisewolfe.com`;
    await page.goto('/sign-up');
  ...
  test('should verify cascade delete removes related records', async ({
    ...
    await page
      .getByLabel('Email')
      .fill(`delete-test-${Date.now()}@tortoisewolfe.com`);
    await page.getByLabel('Password').fill(testPassword);
    await page.getByLabel('Confirm Password').fill(testPassword);
    await page.getByRole('button', { name: 'Sign Up' }).click();
    await page.waitForURL(/\/(verify-email|profile)/);
