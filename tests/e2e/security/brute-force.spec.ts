...
test.describe('Brute Force Prevention - REQ-SEC-003', () => {
  const testEmail = `brute-force-test-${Date.now()}@tortoisewolfe.com`;
  ...
  test('should show remaining attempts counter', async ({ page }) => {
    const uniqueEmail = `attempts-test-${Date.now()}@tortoisewolfe.com`;
  ...
  test('should track different users independently', async ({ browser }) => {
    const userA = `user-a-${Date.now()}@tortoisewolfe.com`;
    const userB = `user-b-${Date.now()}@tortoisewolfe.com`;
  ...
  test('should track different attempt types independently', async ({
    ...
    const email = `types-test-${Date.now()}@tortoisewolfe.com`;
  ...
  test('should not bypass rate limiting by clearing localStorage', async ({
    ...
    const email = `bypass-test-${Date.now()}@tortoisewolfe.com`;
  ...
  test('should display lockout expiration time', async ({ page }) => {
    const email = `lockout-time-${Date.now()}@tortoisewolfe.com`;
