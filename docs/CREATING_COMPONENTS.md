...
This guide explains how to create new components in HogBall following our enforced component structure standards.
...
docker compose exec hogball pnpm run audit:components

# Validate for CI/CD (exits with error if non-compliant)
docker compose exec hogball pnpm run validate:structure
```
...
docker compose exec hogball pnpm run migrate:components
```
...
```bash
docker compose exec hogball pnpm run audit:components -- --verbose
```
