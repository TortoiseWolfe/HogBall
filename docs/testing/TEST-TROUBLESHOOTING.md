...
docker compose exec hogball pnpm run type-check    # TypeScript
docker compose exec hogball pnpm run lint           # ESLint
docker compose exec hogball pnpm run format:check   # Prettier
docker compose exec hogball pnpm test              # Unit tests
docker compose exec hogball pnpm run test:coverage # Coverage
docker compose exec hogball pnpm run test:a11y     # Accessibility
```
...
```bash
docker compose exec hogball pnpm run e2e:accessibility
```
...
docker compose exec hogball node scripts/validate-structure.js
```
