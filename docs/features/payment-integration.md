...
1. Click "New project"
2. Fill in details:
   - **Name**: `hogball-payments` (or your project name)
...
Edit `/home/turtle_wolfe/repos/HogBall/.env` and add:
...
```bash
docker compose exec hogball supabase --version
...
docker compose exec hogball supabase link --project-ref $SUPABASE_PROJECT_REF --password $SUPABASE_DB_PASSWORD
```
...
```bash
# Push migrations
docker compose exec hogball supabase db push
...
```bash
# Generate types file
docker compose exec hogball sh -c 'supabase gen types typescript --linked > /app/src/lib/supabase/types.ts'

# Verify file created
docker compose exec hogball ls -lh /app/src/lib/supabase/types.ts
```
