...
**Constitution File**: `.specify/memory/constitution.md` (HogBall Constitution v1.0.1 - 6 core principles)
...
# Verify
docker compose exec hogball npx supabase db execute \
...
```bash
docker compose exec hogball pnpm add react-easy-crop
```
...
```bash
# Unit tests
docker compose exec hogball pnpm test src/lib/avatar
docker compose exec hogball pnpm test src/components/atomic/AvatarUpload

# E2E tests
docker compose exec hogball pnpm run test:e2e
