...
docker compose exec hogball pnpm exec playwright test e2e/screenshot-current-state.spec.ts
```
...
docker compose exec hogball pnpm exec playwright test e2e/tests/blog-mobile-ux.spec.ts --project="iPhone 12"
...
docker compose exec hogball pnpm exec playwright show-report
...
docker compose logs hogball | grep -i error
```
...
docker compose exec hogball pnpm exec playwright test e2e/screenshot-after-quickfix.spec.ts
...
- [ ] Run `docker compose exec hogball pnpm run build`
- [ ] Build completes without CSS errors
- [ ] Static export generates successfully

### Accessibility Check

- [ ] Run `docker compose exec hogball pnpm run test:a11y:dev`
...
docker compose exec hogball pnpm list tailwindcss
...
docker compose exec hogball pnpm exec playwright install
...
docker compose exec hogball npx lighthouse http://localhost:3000 --preset=mobile --quiet
