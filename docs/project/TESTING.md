...
# Run tests with UI
docker compose exec hogball pnpm test:ui
...
```bash
# .husky/pre-commit
docker compose exec -T hogball pnpm lint-staged
```
...
| Admin | admin@hogball.com | Welcome messages |
...
docker compose exec hogball pnpm exec tsx scripts/seed-connections.ts
```
...
docker compose exec hogball pnpm exec playwright test tests/e2e/messaging/

# Run with existing dev server (faster)
SKIP_WEBSERVER=true docker compose exec -e SKIP_WEBSERVER=true hogball pnpm exec playwright test

# Run specific browser
docker compose exec hogball pnpm exec playwright test --project=chromium
...
docker compose exec hogball pnpm exec playwright test --trace on

# View trace
docker compose exec hogball pnpm exec playwright show-trace test-results/*/trace.zip

# Run in debug mode (step through)
docker compose exec hogball pnpm exec playwright test --debug
```
