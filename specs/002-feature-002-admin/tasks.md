...
- [x] T001 [P] Add admin environment variables to `.env`
  ...
  ```bash
  TEST_USER_ADMIN_EMAIL=admin@hogball.com
...
- [x] T003 [P] Add admin constants to `tests/fixtures/test-user.ts`
  ...
  export const TEST_EMAIL_ADMIN =
    process.env.TEST_USER_ADMIN_EMAIL || 'admin@hogball.com';
...
- [x] T007 Execute migration via Supabase Management API
  ...
  - Updated admin profile with username 'hogball'
...
**Goal**: Admin user "HogBall" exists and can be authenticated
  ...
  describe('Admin User Setup', () => {
    it('should have admin profile in database', async () => {
      const { data } = await supabase
        ...
        .eq('id', ADMIN_USER_ID)
        .single();
      expect(data?.username).toBe('hogball');
    });
  });
  ```
...
- [x] T009 [US3] Verify admin profile exists via Supabase Management API
  ...
  - Confirmed: username='hogball', display_name='HogBall', welcome_message_sent=true
...
**Goal**: New users receive encrypted welcome message from HogBall on first login
...
- [x] T018 [US1] Add welcome message content constant to `src/services/messaging/welcome-service.ts`
  ...
  ```typescript
  export const WELCOME_MESSAGE_CONTENT = `Welcome to HogBall!
