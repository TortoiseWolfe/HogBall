...
```bash
docker compose exec hogball pnpm test src/lib/auth/oauth-utils.test.ts
```
...
```bash
# Unit tests
docker compose exec hogball pnpm test oauth-utils

# E2E test (if available)
docker compose exec hogball pnpm exec playwright test welcome-message
```
