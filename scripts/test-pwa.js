...
async function runPWATests() {
  ...
  try {
    ...
    try {
      ...
      console.error('  docker compose exec hogball pnpm run dev');
      process.exit(1);
    }
    ...
    const results = await page.evaluate(async () => {
      ...
      try {
        if (!('caches' in window)) {
          ...
          const hogballCaches = cacheNames.filter((name) =>
            name.startsWith('hogball-')
          );

          if (hogballCaches.length === 0) {
            testResults.push({
              feature: 'Offline Capability',
              status: 'warning',
            ...
            const cache = await caches.open(hogballCaches[0]);
